...
class UILayout:
    ...
    def __init__(self, width, height):
        ...
        # 背景图
        # self.background1 = pygame.image.load("src/ui/resource/image/background.jpg")
        self.background = pygame.transform.smoothscale(pygame.image.load("src/ui/resource/image/background.jpg"), (self.width, self.height))
        # 字体
        #self.font = pygame.font.Font("src/ui/resource/font/ArtierEN-2.ttf", 36)
    ...
    def draw_ui(self, surface, score, combo, missed, game_time):
        ...
        score_surface = self.font_manager.render_score_life_text(score_text, (255, 255, 255))
        ...
        # 绘制生命，用艺术X表示
        missed_text = f"X " * (int(GameConfig.MAX_MISSED_FRUITS) - int(missed))
        missed_surface = self.font_manager.render_score_life_text(missed_text, (255, 0, 0))
        missed_rect = missed_surface.get_rect(topleft=self.ui_positions['missed'])
        surface.blit(missed_surface, missed_rect)
